...
export class AuthService {
  ...
  async login(user: CreateUserDto) {
    ...
    const alreadyExistingUser = await this.userService.findByEmail(email,{roles: true});
    ...
    return this.generateToken({
      ...
      email,
      roles: alreadyExistingUser.roles.map(rol => rol.id)
    });
  }
  ...
  async register(user: CreateUserDto) {
    ...
    await this.userService.createUser(user,{id: RoleIds.Customer} );
